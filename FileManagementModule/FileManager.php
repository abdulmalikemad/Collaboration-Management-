...
public function getFilesByTask($taskId) {
  ...
  // 5. حذف ملف الطالب (deleteTaskFile)
  public function deleteTaskFile($fileId, $studentId) {
    // التحقق من ملكية الملف
    $stmt = $this->conn->prepare("SELECT file_path FROM task_files WHERE id = ? AND student_id = ?");
    ...
    if ($file) {
      ...
      $deleteStmt = $this->conn->prepare("DELETE FROM task_files WHERE id = ? AND student_id = ?");
      $deleteStmt->bind_param("ii", $fileId, $studentId);
